...
Plugin.registerSourceHandler 'browserify.js', (step) ->
  ...
  stream = new stream.PassThrough()   # hold Buffer as Readable stream
  stream.end step.read()              # put Buffer into stream
  basedir = step.fullInputPath.slice(0, -(step.inputPath.length)) + '.npm/package'
  browserify = Browserify [stream], debug:true, basedir:basedir
